    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 overflow-y-auto backdrop-blur-sm">
      ...
      >
        ...
        <div className="px-6 pb-6 overflow-y-auto" style={{ maxHeight: 'calc(100vh - 16rem)' }}>
          ...
          {/* Step 1: Configure — all providers now follow the same backend-only storage flow */}
...
// Deprecated: 旧的新手钱包引导表单仅为回滚保留，当前离线认证改造后不再进入主流程。
void Claw402ConfigForm
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-5">
      ...
      <div className="flex gap-3 pt-4">
        ...
        <button
          type="submit"
          disabled={!selectedModel || (!apiKey.trim() && !editingModelId)}
          className="flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-sm font-bold transition-all hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed"
