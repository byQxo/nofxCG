...
func (s *StrategyStore) Create(strategy *Strategy) error {
	if err := strategy.prepareConfigForStorage(); err != nil {
		return err
	}
...
func (s *Strategy) ParseConfig() (*StrategyConfig, error) {
	...
	if err := decryptStrategySecrets(&config); err != nil {
		return nil, err
	}
	return &config, nil
}
...
func (s *Strategy) SetConfig(config *StrategyConfig) error {
	...
	storageCopy := *config
	if err := encryptStrategySecrets(&storageCopy); err != nil {
		return err
	}
	data, err := json.Marshal(&storageCopy)
	if err != nil {
...
func (s *Strategy) prepareConfigForStorage() error {
...
// RedactedConfig 返回适合前端展示的策略配置副本。
// 敏感字段不会回传，前端只通过是否已配置来感知状态。
func (s *Strategy) RedactedConfig() (*StrategyConfig, error) {
...
func encryptStrategySecrets(config *StrategyConfig) error {
	if config == nil {
		return nil
	}
	service := crypto.GetGlobalCryptoService()
	if service == nil {
		return nil
	}
	key := strings.TrimSpace(config.Indicators.NofxOSAPIKey)
	if key == "" || service.IsEncryptedStorageValue(key) {
		return nil
	}
	encrypted, err := service.EncryptForStorage(key, "strategy", "nofxos_api_key")
	if err != nil {
		return fmt.Errorf("failed to encrypt strategy secret: %w", err)
	}
	config.Indicators.NofxOSAPIKey = encrypted
	return nil
}

func decryptStrategySecrets(config *StrategyConfig) error {
	if config == nil {
		return nil
	}
	service := crypto.GetGlobalCryptoService()
	...
	decrypted, err := service.DecryptFromStorage(key, "strategy", "nofxos_api_key")
	if err != nil {
		return fmt.Errorf("failed to decrypt strategy secret: %w", err)
	}
	config.Indicators.NofxOSAPIKey = decrypted
	return nil
}
