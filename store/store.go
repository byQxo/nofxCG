...
func (s *Store) initTables() error {
	...
	if err := s.AuthSession().initTables(); err != nil {
		return fmt.Errorf("failed to initialize auth session tables: %w", err)
	}
	return nil
}
...
// AuthSession 获取持久化会话存储。
func (s *Store) AuthSession() *AuthSessionStore {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.authSession == nil {
		s.authSession = NewAuthSessionStore(s.gdb)
	}
