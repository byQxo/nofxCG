...
// getSafeStrategyConfig 将策略配置解析为适合前端返回的安全副本。
// 这里会移除敏感字段，例如 nofxos_api_key，避免配置接口回显密钥。
func getSafeStrategyConfig(strategy *store.Strategy) store.StrategyConfig {
	if strategy == nil {
		return store.StrategyConfig{}
	}
	config, err := strategy.RedactedConfig()
...
func (s *Server) handlePublicStrategies(c *gin.Context) {
	...
	for _, st := range strategies {
		...
		if st.ConfigVisible {
			item["config"] = getSafeStrategyConfig(st)
		}
...
func (s *Server) handleGetStrategies(c *gin.Context) {
	...
	for _, st := range strategies {
		result = append(result, gin.H{
			...
			"is_public":      st.IsPublic,
			"config_visible": st.ConfigVisible,
			"config":         getSafeStrategyConfig(st),
			"created_at":     st.CreatedAt,
			"updated_at":     st.UpdatedAt,
		})
	}
...
func (s *Server) handleGetStrategy(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{
		...
		"config":      getSafeStrategyConfig(strategy),
